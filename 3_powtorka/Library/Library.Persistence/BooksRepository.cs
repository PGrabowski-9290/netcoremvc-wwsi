...
namespace Library.Persistence
{
    public class BooksRepository
    {
        ...
        {
            _database.Remove(_database.First(book => book.Title == title));
        }
        public void ChangeState(string title, int changeState)
        {
            var index = _database.FindIndex(book => book.Title == title);
            _database[index].ProductAvailable += changeState;

            _database.First(book => book.Title == title).ProductAvailable += changeState;
        }
